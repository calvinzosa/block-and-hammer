...
replayGui.GetPropertyChangedSignal('Visible').Connect(() => {
	const currentArea = getCurrentArea(Workspace.FindFirstChild(`cube${player.UserId}`));
    if (replayGui.Visible && (currentArea === 'ErrorLand' || currentArea === 'Tutorial')) {
...
function viewReplay(userId: number, frames: string[]) {
	...
	loadAccessories(replayCube, { 'hat': cubeMetadata[0], 'face': cubeMetadata[1], 'aura': cubeMetadata[2], 'hammer': cubeMetadata[3] }, player, undefined);
	...
	let winFrameIndex = undefined as (number | undefined);
	let totalWinTime = undefined as (number | undefined);
	...
	updateEvent = RunService.Heartbeat.Connect((dt) => {
...
(uploadConfirmation.WaitForChild('Yes') as TextButton).MouseButton1Click.Connect(() => {
	...
	let totalChunks = math.ceil(compressedData.size() / chunkSize);
	let chunkNumber = 1;
