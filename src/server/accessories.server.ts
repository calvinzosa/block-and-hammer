import { ReplicatedStorage, BadgeService, RunService, Workspace, Players, TweenService, Debris } from '@rbxts/services';

import { Accessories, PlayerAttributes } from 'shared/utils';
...
Events.BuildingHammerPlace.OnServerEvent.Connect((player, position, buildType) => {
	...
	TweenService.Create(part, new TweenInfo(0.5, Enum.EasingStyle.Linear), {
		Transparency: 0,
	}).Play();
	TweenService.Create(label, new TweenInfo(0.5, Enum.EasingStyle.Linear), {
		TextTransparency: 0,
	...
	TweenService.Create(part, new TweenInfo(1, Enum.EasingStyle.Linear), {
		Transparency: 1,
	}).Play();
	TweenService.Create(label, new TweenInfo(1, Enum.EasingStyle.Linear), {
		TextTransparency: 1,
